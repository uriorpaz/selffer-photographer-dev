IntercomRails.config do |config|
  # == Intercom app_id
  #
  config.app_id = ENV["INTERCOM_APP_ID"] || "c7kqksbj"
  ...
  #
config.user.current = Proc.new { current_photographer || @photographer }
  ...
  #
  config.user.model = Proc.new { Photographer }
  ...
  config.user.custom_data = {
      :photographer => Proc.new { |user| !user.is_owner },
      :owner => Proc.new { |user| user.is_owner }
  }
